...
Phase 1, Phase 2, Phase 3, Phase 4, Phase 5, Phase 6, Phase 6.5 stabilization, and Phase 7 are complete. Phase 7 turned user messages during active execution into real interrupt/replan checkpoints and added v2 requirement-ledger revision handling for append, modify, replace, cancel, and approval revision checks.

Important handoff for Phase 8: Phase 7 did not switch production defaults to v2, did not remove legacy paths, and did not perform legacy cleanup. `pending_user_message` remains a compatibility response/model field, but the `EXECUTING` message path no longer stores user changes there; it writes a controller checkpoint into `replan_context` and clears the field.
