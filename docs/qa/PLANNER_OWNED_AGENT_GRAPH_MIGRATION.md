...
- At graph-migration start, the old `factory-agent/factory_agent/graph/` scaffold still contained historical LangGraph-style concepts such as `working_intents`, `intent_cursor`, and `intent_completed`. Legacy cleanup Phase 3.8 deleted that scaffold; those terms now remain only as historical migration context, static guard vocabulary, compatibility schema values, or frontend release-harness vocabulary. They must not become execution authority again.
