...
PROMPT_CONTRACTS: tuple[PromptContract, ...] = (
    ...
    PromptContract(
        name="summary_plan_langchain_v1",
        version="v1",
        status="optional",
        purpose="Optionally rewrite execution-plan summaries when SUMMARY_BACKEND resolves to langchain.",
        inputs=("intent", "plan_explanation", "risk_summary"),
        output_contract="operator_plan_summary_text",
        validator="non_empty_text",
        fallback="factory_agent.analysis.summary_backend.SummaryAdapter._summarize_deterministic",
        owner_module="factory_agent.analysis.summary_backend",
    ),
    PromptContract(
        name="legacy_planner_prompt_v1",
        version="v1",
        status="legacy",
        purpose="Legacy whole-plan JSON prompt kept for compatibility tests; planner-owned graph uses planner_decision_v2.",
        inputs=("user_goal", "tools_markdown", "scoped_tool_names"),
        output_contract="PlanDraft JSON",
        validator="factory_agent.llm.plan_parsing.parse_plan_json",
        fallback="planner_owned_agent_graph",
        owner_module="factory_agent.planning.prompting",
    ),
    PromptContract(
        name="legacy_rag_rerank_v1",
        version="v1",
        status="legacy",
        purpose="Compatibility JSON prompt for deployments that inject an LLM reranker instead of the BGE reranker.",
        inputs=("query", "route", "candidate_ids"),
        output_contract="ordered_chunk_id_list_json",
        validator="json.loads list[str] filtered to known candidate ids",
        fallback="input candidate order",
        owner_module="factory_agent.rag.reranking",
    ),
)
