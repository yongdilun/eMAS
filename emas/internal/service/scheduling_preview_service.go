...
type SolverPreviewStep struct {
	...
	RoundingOverheadMins   int                `json:"rounding_overhead_mins"`
...
func (s *SchedulingService) BuildSolverPreview(jobID string) (*SolverPreview, error) {
	return s.buildSolverPreview(jobID, nil, nil)
}

func (s *SchedulingService) BuildSolverPreviewWithEarliestStart(jobID string, earliestFloor *time.Time) (*SolverPreview, error) {
	return s.buildSolverPreview(jobID, nil, earliestFloor)
}
...
func (s *SchedulingService) buildSolverPreview(jobID string, tentativeSlots []TentativeSlot, earliestFloor *time.Time) (*SolverPreview, error) {
	...
	if earliestFloor != nil && earliestFloor.After(cursor) {
		cursor = alignSuccessorStart(*earliestFloor)
	}
	earliestReadyAt := func() *time.Time {
		if readiness == nil {
			return nil
		}
		if readiness.EarliestReadyAt == nil {
			return nil
		}
		aligned := alignSuccessorStart(*readiness.EarliestReadyAt)
		return &aligned
	}()
	...
	for _, jobStep := range orderedSteps {
		...
		durationMetrics := stepDurationMetrics(*processStep, candidates, float64(jobStep.QuantityTarget))
		candidates, err = s.CandidateMachinesForStepWithTentative(jobStep.JobStepID, cursor, cursor.Add(durationMetrics.ReservedDuration), tentativeSlots)
		if err != nil {
			return nil, err
		}
...
func (s *SchedulingService) BuildSolverPreviewWithTentativeSlots(jobID string, tentativeSlots []TentativeSlot) (*SolverPreview, error) {
	return s.buildSolverPreview(jobID, tentativeSlots, nil)
}

func (s *SchedulingService) BuildSolverPreviewWithTentativeSlotsAndFloor(jobID string, tentativeSlots []TentativeSlot, earliestFloor *time.Time) (*SolverPreview, error) {
	return s.buildSolverPreview(jobID, tentativeSlots, earliestFloor)
}
