...
const UtilizationChart = ({ machines = [], utilizationData = null }) => {
  ...
  return (
    <div className="mb-4 bg-white dark:bg-[#111618] rounded-xl border border-gray-300 dark:border-[#3b4e54] p-5">
      <div className="flex flex-col gap-2">
        <p className="text-gray-900 dark:text-white text-sm font-semibold leading-normal">
          Overall Machine Utilization
        </p>
        <div className="flex items-baseline gap-2">
          <p className="text-gray-900 dark:text-white text-[28px] font-bold leading-tight">{avg}%</p>
          <p className="text-green-600 dark:text-[#0bda57] text-sm font-medium flex items-center">
        ...
        <div className="flex flex-col gap-2 pt-2">
          {rows.map((row) => (
            <div key={row.name} className="flex items-center gap-3">
              <p className="text-gray-600 dark:text-[#9cb3ba] text-xs w-28 shrink-0 truncate text-right">{row.name}</p>
              ...
              <span className="text-[11px] text-gray-500 dark:text-gray-400 w-8 text-right">{row.pct}%</span>
            </div>
          ))}
        </div>
      </div>
