...
import { seededArtifactDir } from './fullStackEnv.js'
...
export const test = base.extend({
  page: async ({ page }, use, testInfo) => {
    ...
    if (testInfo.status !== testInfo.expectedStatus) {
      await attachSeededStackArtifacts(testInfo)
    }
  },
})
...
export async function attachSeededStackArtifacts(testInfo) {
  ...
  await attachFileIfExists(testInfo, path.join(artifactDir, 'seeded-stack.log'))
}
