...
try {
  ...
  startProcess('vite', process.execPath, [
    ...
    env: {
      PLAYWRIGHT_SEEDED_ARTIFACT_DIR: env.artifactDir,
    },
  })
  await waitForText(env.viteBaseUrl, { timeoutMs: 60_000, label: 'Vite' })
  append(path.join(env.artifactDir, 'seeded-stack.log'), `[${new Date().toISOString()}] seeded stack ready\n`)
} catch (err) {
  append(path.join(env.artifactDir, 'seeded-stack.log'), `[${new Date().toISOString()}] startup failed: ${err?.stack || err}\n`)
  stopAll()
  throw err
}
