...
Status: COMPLETE. Evidence: `factory-agent/factory_agent/graph/session_detection.py` now centralizes graph-native session detection across LangGraph-created compatibility plans, `langgraph_pending_approval` context, and durable `langgraph_native_checkpoint` rows so API routes and `main.py` worker/replay guards classify checkpoint-only graph sessions the same way snapshot does. `factory-agent/factory_agent/orchestration/execution.py` now labels `ExecutionEngine` as legacy relational plan-step compatibility and refuses `execute_until_blocked` for any graph-native session as a final guardrail. `factory-agent/factory_agent/orchestration/execution_runtime.py` and `factory-agent/factory_agent/orchestration/agent_integration.py` are explicitly labeled compatibility-only; `factory-agent/tests/test_execution_engine.py`, `factory-agent/tests/test_approval_resume_integration.py`, and `factory-agent/tests/test_phase5_agent_integration.py` are marked `legacy_compatibility` with marker registration in `factory-agent/pyproject.toml`. `factory-agent/tests/test_phase8_legacy_retirement.py` proves checkpoint-only graph sessions do not fall back to the legacy engine, the legacy engine rejects graph-native checkpoint sessions directly, and production code has no `QueryRouter` imports outside the deprecated router module. Verification run on 2026-05-13: `python -m pytest tests/test_phase8_legacy_retirement.py -q`, `python -m pytest tests/test_agent_state.py tests/test_intent.py tests/test_intent_splitter.py tests/test_planner_phase3.py tests/test_tool_pipeline.py tests/test_phase5_final_validator.py tests/test_planner_service_phase6.py tests/test_phase7_api_ui_alignment.py tests/test_phase8_legacy_retirement.py -q`, `python -m pytest tests/test_approval_resume_integration.py tests/test_phase5_agent_integration.py -q`, `python -m pytest tests/test_execution_engine.py --collect-only -q`, and `python -m compileall factory_agent main.py`.
...
Final legacy report: `SessionRow`, `MessageRow`, `PlanRow`, and `PlanStepRow` remain for public API/UI history and legacy-session compatibility; `ExecutionEngine`, worker/cold-start recovery, step approvals, and DLQ replay remain active only for non-graph legacy sessions and are blocked for graph-native sessions by shared graph-native detection plus the direct legacy-engine guard; `QueryRouter` remains deprecated/eval compatibility and is not imported by graph-native production runtime; the frontend `createPlan -> execute` flow remains a documented future behavior-changing cleanup, with snapshot hydration and semantic SSE recovery already aligned. End-to-end evidence covers read flow and planner loop (Phases 3-4), write staging/dry-run/final validation/approval/atomic commit (Phase 5 plus backend transaction tests), approval pause/resume and durable recovery (Phase 6), SSE and frontend hydration (Phase 7 plus frontend build), and graph-native legacy-retirement guardrails (Phase 8 plus final API/broad suites).
